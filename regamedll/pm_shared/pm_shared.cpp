...
void EXT_FUNC __API_HOOK(PM_Duck)()
{
	...
	|| prevent_duck_during_freezetime.value && g_pGameRules->IsFreezePeriod()) // Prevent ducking during freezetime if the prevent_duck_during_freezetime cvar is set
	{
		// Try to unduck
...
void EXT_FUNC __API_HOOK(PM_Jump)()
{
	...
	|| prevent_jump_during_freezetime.value && g_pGameRules->IsFreezePeriod()) // Prevent jumping during freezetime if the prevent_jump_during_freezetime cvar is set
	{
		return;
	}
#endif
