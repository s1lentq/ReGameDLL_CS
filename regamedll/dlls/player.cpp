...
void CBasePlayer::HandleSignals()
{
	if (CSGameRules()->IsMultiplayer())
	{
...
#endif
		{
			...
			{
				if (pev->deadflag == DEAD_NO && (m_iTeam == TERRORIST || m_iTeam == CT) && !(m_signals.GetSignal() & SIGNAL_BUY) 
					// Restricted by map rules
					&& CSGameRules()->CanPlayerBuy(this)
				)
				{
					if (buy_anywhere.value == 1.0f 
						|| (buy_anywhere.value == 2.0f && m_iTeam == TERRORIST) 
						|| (buy_anywhere.value == 3.0f && m_iTeam == CT)
					)
					{
